...
export function OddsComparisonTable({ comparison }: OddsComparisonProps) {
  const getValueBadge = (value: { hasValue: boolean; valuePercentage: number } | null) => {
    ...
    return <Badge className={`${color} border-0 text-xs`}>+{(Number(value.valuePercentage) || 0).toFixed(1)}%</Badge>
  }
            ...
            <TableRow key={index}>
              ...
              <TableCell className="text-center">
                <div className="space-y-1">
                  <p className="font-medium">{(Number(odds.home_odds) || 0).toFixed(2)}</p>
              ...
              {comparison.prediction.draw_probability && (
                <TableCell className="text-center">
                  <div className="space-y-1">
                    <p className="font-medium">{odds.draw_odds ? (Number(odds.draw_odds) || 0).toFixed(2) : "-"}</p>
              ...
              <TableCell className="text-center">
                <div className="space-y-1">
                  <p className="font-medium">{(Number(odds.away_odds) || 0).toFixed(2)}</p>
